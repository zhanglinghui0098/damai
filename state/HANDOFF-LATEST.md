...
| `state/PLAN-CANVAS-MIGRATION.md` | **11KB / ⭐ 06-29 19:30 新** | **画布迁移 @xyflow/react 计划 (4 phase, 8-10h, ~10 commit), 下 session 第一件事** |
...
1. **🚨 第一优先: 画布迁移到 @xyflow/react** (06-29 19:30 user 决定, 走方案 A)
   - 完整计划: `state/PLAN-CANVAS-MIGRATION.md` (11KB, 4 phase, 8-10h, ~10 commit)
   - Phase 1: 安装 + 脚手架 (1h)
   - Phase 2: 核心迁移 (4-5h, 6 节点 + 边 + state + 模板 + 5 交互)
   - Phase 3: UI 壳子集成 (2-3h, TopBar/FloatingTools/Zoom/Properties/TextToolbar 改 binding)
   - Phase 4: 测试 + 部署 (1-2h)
2. **P0 #5 verify-code 收口**: 跑 `curl POST /api/auth/verify-code` 用 user 贴的 6 位数
3. **push 落后 4 commit** (本 session 已推, 保持同步)
4. **P1 #2.1 S4**: dashboard ProjectsTab 接真 Bitable (替换 mock-data-workbench)
5. **user 硬刷测画布** (迁移后): Ctrl+Shift+R 测 bezier 平滑 / 箭头方向 / 节点位置 100% 准
6. **讨论技术问题** (user 19:15 说要新开 session 讨论, 19:30 已选 A 走起)
